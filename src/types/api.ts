// To parse this data:
//
//   import { Convert, APIScapeXResponse } from "./file";
//
//   const aPIScapeXResponse = Convert.toAPIScapeXResponse(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface APIScapeXResponse {
    docs:          Doc[];
    totalDocs:     number;
    offset:        number;
    limit:         number;
...
export class Convert {
    public static toAPIScapeXResponse(json: string): APIScapeXResponse {
        return cast(JSON.parse(json), r("APIScapeXResponse"));
    }

    public static aPIScapeXResponseToJson(value: APIScapeXResponse): string {
        return JSON.stringify(uncast(value, r("APIScapeXResponse")), null, 2);
    }
}
